...
balance_table <- data.frame(balance_covar_df) %>%
  ...
  select(1:3)
gt::gt(balance_table, caption = paste("chisquare:", round(balance_covar_df$overall.chisquare, 3), "df:", balance_covar_df$overall.df, "p.value:", balance_covar_df$overall.p.value))
```
...
# Balance Table ...change this to the other format used earlier
balance_covar_match_df <- as.data.frame(balance_covar_match)
sjPlot::tab_df(balance_covar_match_df)
...
**The coefficient indicates that on average families who participated in the program, had their animal holdings increase by 256.95 pesos according to the survey recorded in 1999 (p-value 0.002).**
...
**The coefficient indicates that on average family who participated in the program, had their animal holdings increase by 160.59 pesos according to the survey recorded in 1999 as compared to a family who did not participate in the program (p-value < 0.01).**
...
The fixed effect estimator solves a problem in omitted variables bias by introducing a dummy vairable in order to account for variables that change over time but do not change over the observational unit. The first difference estimator solves a problem in omitted variable bias by 
